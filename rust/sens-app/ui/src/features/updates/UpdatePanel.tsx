...
function UpdatePanel() {
  ...
  const [said, setSaid] = useState({ text: installable ? "" : t.devBuild, failed: false });
  ...
  async function install() {
    if (!sure) {
      ...
      if (working > 0) {
        setSure(true);
        setSaid({ text: t.working(working), failed: false });
        return;
      }
    }
    setBusy(true);
    setSaid({ text: t.stage.downloading, failed: false });
    try {
  ...
  const status = busy && stage ? t.stage[stage] : said.text;
  return (
    <>
      <p className="note">{[current && t.youHave(current), weigh(latest.size)].filter(Boolean).join(" · ")}</p>
      {latest.notes.trim() ? <Markdown className="update-notes" text={latest.notes} /> : <p className="note update-notes">{t.noNotes}</p>}
      ...
      <div className="actions update-actions">
        ...
        <button className={sure ? "primary danger" : "primary"} disabled={!installable || busy} onClick={install}>
          {sure ? t.updateAnyway : again ? shared.retry : t.updateRestart}
        </button>
      </div>
    </>
  );
